def calculate_PMF(dist, ax):
    ...
    hist, bin_edges = np.histogram(dist, bins=num_bins, density=True)
...
def create_combined_plots():
    ...
    for i, pKa_column in enumerate(pKas.columns):
        ...
        for j, dist_col in enumerate(distcance_columns):
            ...
            ax[x, 0].plot(
                distances.index / frame_to_time, distances[dist_col], color=dist_color
            )
            ax[x, 0] = ax_util(
                ax[x, 0],
            ...
            ax[x, 3] = ax_util(ax[x, 3], xlabel="PMF (kcal/mol)", ylabel="Distance (Å)")

...
warnings.filterwarnings("ignore", category=UserWarning, module="propkatraj.*")
...
class DNetPKa:
    ...
    def compute_pka_for_traj(
        self, selection="protein", start=None, stop=None, step=None
    ):
        """
    ...
    def plot_pka_time_series_for_selection(
        self, selection=None, write_to_file=None, figsize=None
    ):
        if self.pkas.empty:
            print("No pKa data available. Please run compute_pka_for_traj first.")
            return None, None
        ...
        fig, ax = self._create_plot(
            title=title, xlabel="Time (ns)", ylabel=r"p$K_a$", figsize=figsize
        )
    ...
    def plot_pka_statistic_for_selection(
        self, selection=None, write_to_file=None, figsize=None
    ):
        if self.pkas.empty:
            print("No pKa data available. Please run compute_pka_for_traj first.")
        ...
        title = selection if selection else self.selection
        fig, ax = self._create_plot(
            title=title, xlabel="Res ID", ylabel=r"p$K_a$", figsize=figsize
        )
    ...
    def write_pka_to_external_data_file(self, write_to_file):
        if self.pkas.empty:
            print("No pKa data available. Please run compute_pka_for_traj first.")
            return None
        assert write_to_file.endswith(
            "_data.txt"
        ), "Name of external data file should end with _data.txt"
    ...
    def write_last_frame_as_pdb(self, write_to_file):
        ...
        with mda.Writer(
            write_to_file, multiframe=True, bonds=None, n_atoms=self._u.atoms.n_atoms
        ) as PDB:

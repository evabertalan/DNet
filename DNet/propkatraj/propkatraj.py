...
class PropkaTraj(AnalysisBase):
    ...
    def __init__(self, atomgroup, select="protein", skip_failure=False, **kwargs):
        ...
        if len(self.ag.select_atoms("not protein")) > 0:
            wmsg = (
                "Non protein atoms passed to propka 3.1.\n MDAnalysis' "
    ...
    def _conclude(self):
        ...
        if self.results.num_failed_frames > 0:
            ...
            failed_indices = [
                np.where(self.frames == i) for i in self.results.failed_frames_log
            ]

            self.times = np.delete(self.times, failed_indices)
            self.frames = np.delete(self.frames, failed_indices)
            self.failed_indices = failed_indices

            wmsg = (
                "number of failed frames = {0}\n"
                "percentage failure = {1}\n"
                "failed frames: {2}".format(
                    self.results.num_failed_frames,
                    perc_failure,
                    self.results.failed_frames_log,
                )
            )

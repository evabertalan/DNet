...
import propkatraj
from propkatraj.tests.datafiles import (
    PSF_FRAME_ZERO_PKA,
...
@pytest.fixture(scope="module")
def glu_ref():
    arr = np.array([2.53, 2.61, 2.84, 2.57, 2.76, 2.51, 2.68, 2.95, 3.18, 3.34])
    return arr


def pka_from_file(filename):
    columns = ["resname", "resnum", "chain", "pka", "model-pka"]
    df = pd.read_csv(filename, names=columns, skiprows=1, delim_whitespace=True)
...
def test_single_frame_regression(tmpdir, u, selection):
    ...
    with tmpdir.as_cwd():
        ...
        # test pka values
        assert_almost_equal(pkatraj.results.pkas.values[0], ref_pkas, decimal=2)
...
@pytest.mark.parametrize("start, stop, step", [(None, 5, None), (None, None, 25)])

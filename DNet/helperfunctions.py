...
def create_logger(folder):
    ...
    if not len(logger.handlers):
        ...
        fh = logging.FileHandler(Path(folder, "cgraphs_logs.log"))
        with open(Path(folder, "cgraphs_logs.log"), "w"):
            pass
...
def write_text_file(file_path, text_content, logger=None):
    ...
    if str(file_path).endswith(".txt") and isinstance(text_content, list):
        f = open(file_path, "w")
...
def get_water_coordinates(protein_chain, res_index):
    ...
    if len(sel.positions):
        ...
        print(
            "Water "
            + res_index
            + " not found in the PDB file. INFO: https://github.com/evabertalan/cgraphs/blob/main/README.md"
        )
        return None

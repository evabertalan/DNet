...
class CompareTwo(ProteinGraphAnalyser):
    ...
    def plot_graph_comparison(
        ...
        if len(self.graph_coord_objects.items()) != 2:
            self.logger.warning(
                "There are "
                + str(len(self.graph_coord_objects.items()))
                + " structures selected. Graph comparison is possible for exactly two structures."
            )
        else:
            self.logger.info(
                "Plot comparison "
            ...
            if (
                ...
                conserved_waters = {
                    k: v for k, v in conserved_waters.items() if v["conserved"] == True
                }
        ...
        if color_propka and color_data:
            ...
            try:
                color_info = _hf.read_propka_file(
                    f"{self.target_folder}/{name}.propka", selected_nodes
                )
            except:
                self.logger.info(

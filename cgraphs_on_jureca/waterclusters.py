...
class WaterClusters(ProteinGraphAnalyser):
        ...
        for _eps in eps_candidates:
            ...
            if n_clusters_ < 2:
                self.logger.warning(
                    "With eps: "
                    + str(np.round(_eps, 2))
                    + " only 1 water cluster was found. Result of water clutering might be unreliable."
                )
    ...
    def evaluate_parameters(self, eps=1.4, min_samples=None):
        ...
        if eps < 1.0 or eps > 1.6:
            self.logger.warning(
                "For water cluster analysis the recommanded eps is 1.4. See parameter study in supporting information of: https://www.sciencedirect.com/science/article/pii/S1047847720302070"
            )
        ...
        fig, ax = _hf.create_plot(
            title="Projection of the "
            + str(self.n_clusters_)
            + " water clusters and "
            + str(n_noise_)
            + " outlier points",
